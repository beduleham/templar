  ...
  for (let i = 0; i < 16; i++) {
    ...
    for (let j = 0; j < near.length; j++) {
      ...
      if (style === "melee") {
        ...
        if (d < 95) s -= 9;                        // 붙으면 죽는다
        /* 사거리 안이면 이득 — 다만 **몇 마리까지** 이득으로 칠지는 손잡이가 정한다.
           한 마리당 +1.1 이 무한히 쌓이면 200u 에 아홉 마리가 90u 에 붙은 한 마리
           (-9)를 이긴다. 기본은 무한(예전 그대로) — 값을 보고 정한다. */
        else if (d < 280) { if (kcnt < botCfg.rangeCap) s += 1.1; kcnt++; }
      }
    }
    /* 가장 가까운 적 하나로 결정하는 거부권. 위의 -9 는 마릿수에 희석되지만
       이것은 안 희석된다 — 「제일 가까운 놈이 얼마나 가까운가」만 본다. */
    if (style !== "melee" && dmin < botCfg.nearVeto)
      s -= botCfg.nearVetoW * (botCfg.nearVeto - dmin) / botCfg.nearVeto;
...
globalThis.botCfg = globalThis.botCfg || {
  ...
  /* 위험 항과 겨루는 값이라 **같이 움직여야 한다.** §153 에서 거부권을 더해 위험이
     -9 에서 -23 까지 커지자 7 로는 눌려서, god-mode 전직이 4/4 에서 3/4 로, 판당 제단이
     4.5 에서 2.8 로 떨어졌다. 7·14·24·40 을 재니 14 에서 4/4 · 7차 · 4.8 로 돌아온다
     (24·40 도 비슷하지만 넷씩이라 구분은 안 된다). 위험이 두 배가 됐으니 끌림도 두 배다. */
  seekPull: 14,         // 그 끌림의 세기 — 적 점수와 겨룬다
  /* ── 걸음걸이 (§153) ──
     봇은 **자기를 때린 적 쪽으로 걸어 들어가고** 있었다. 접촉 피해의 43~61% 가 그랬고,
     맞는 순간 겹친 적은 평균 1.0 마리에 빈 길이 열여섯 중 아홉이었다 — 둘러싸여 죽는
     것이 아니라 **비어 있는 사방을 두고 한 마리에게 긁혀** 죽고 있었다.

     범인은 점수판의 저울이다. 「사거리 안이면 +1.1」이 마릿수만큼 무한히 쌓이는데
     「붙으면 -9」는 한 마리당 한 번이라, 200u 에 아홉 마리가 90u 에 붙은 한 마리를
     이긴다. 3분이면 동시 적이 마흔이 넘으므로 이 일은 늘 일어난다.

     고친 뒤(n=24): 생존 3:25 → 4:33, 때린 쪽으로 걸은 비율 49% → 30%.
     상한 값 자체는 2·3·4·6 이 4:28 / 4:45 / 4:33 / 4:37 로 전부 표준오차(±11~13초)
     안이다 — **값이 아니라 상한이 있다는 것**이 고친 것이다. 가운데인 4 를 쓴다.
     거부권만 켜면 3:41 로 잡음 안이고, 상한과 같이 켤 때만 4:33 이 된다. */
