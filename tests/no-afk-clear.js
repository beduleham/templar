/* 회귀: 가만히 서서 스킬만 눌러서는 못 이긴다.
   ...
   ■ 아래쪽 조건(2분 안 죽음)이 흔들렸던 원인 — 봇이 도발을 집었다 (2026-09-07)

   같은 코드가 「3판 중 2판이 2분 안에 죽었다」로 이따금 떨어졌다. 난수 탓으로 두고
   다시 돌리기만 했는데, 죽은 판마다 고른 카드를 적어 보니 원인은 하나였다.

     3레벨 특성 판에서 봇이 **도발**을 집은 판    30판 중 9판 → 전부 80~98초에 죽음
     도발을 6레벨에 집은 판                      6판 → 129~147초에 죽음
     도발을 안 집은 판                           15판 → 전부 200초까지 삶(각성까지 감)
     도발을 피하게 하고 24판                     0판 죽음 / 도발을 우선 집게 하고 12판  7판 죽음

   도발은 방벽 중에 곁의 적을 끌어당긴다. 3레벨 · 1레벨 무기 하나로 서 있는 성기사가
   30마리를 제 발로 불러들이면 90초에 죽는 것이 맞다 — 그건 도발이 하는 일이고, 이 자가
   물으려는 것(회복 고리로 서서 이기는가)이 아니다. 봇은 특성 판이 오면 세 장 중
   **첫 장**을 집었고, 첫 장이 도발일 확률이 그대로 실패 확률이었다(판마다 ~30%,
   셋 중 둘이면 ~20%). 그래서 **첫 각성 전에는 도발을 피한다.** 각성 뒤(체력 475~)엔
   집어도 살고, 서서 이기는 가장 센 조합(도발 + 성전 + 회복 고리)을 자에서 빼면 위쪽
   조건이 물러지므로 그때부터는 막지 않는다.
   ...
   ■ 불멸의 성벽이 이긴 판 — 회복 고리는 스킬이 아니라 **피의 낫**이었다
    ...
    for (let n = 0; n < 3; n++) {
      ...
      for (let i = 0; i < 60 * 905; i++) {
        ...
        while ((Game.state === 'levelup' || Game.state === 'advance') && g++ < 60) {
          ...
          /* 특성 판: 첫 각성 전에는 도발을 피한다(머리 주석). 그 뒤엔 무엇이든. */
          const trait = C.find(c => c.type === 'trait' && (c.key !== 'taunt' || player.advance.length > 0));
          Game.applyChoice(C.find(c => c.tier && LINE.includes(c.key)) || C.find(c => c.tier)
            || (hpF < .4 && C.find(c => c.type === 'heal'))
            || C.find(c => c.type === 'passive') || trait || C.find(c => c.type !== 'heal') || C[0]);
        }
