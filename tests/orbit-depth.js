...
(async () => {
  ...
  r.frames.forEach((f, i) => {
    ...
    /* 기준 12 → 9. §100 에서 주인공을 칸 가운데로 옮기며(성기사 ox −5, 화면 2px) 투구와 성서가
       겹치는 자리가 조금 줄어 1장이 11% 로 내려왔다(2~4장 15%). 진짜 검사는 위의 깊이 순서고
       이 값은 보조다 — 0 근처(가려지지 않음)와 11~15(가려짐)를 가르는 데 9 면 충분하다. */
    if (hid < .09) { console.log(`  !! ${i + 1}장 — 뒤쪽 성서가 주인공에게 가려지지 않는다`); bad++; }
  });
