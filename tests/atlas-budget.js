...
/* ■ 문턱을 15000 → 15600 으로 올렸다 (2026-09-06) — 그리고 왜 그래도 괜찮은가
   ...
   ■ 다음에 부딪히면 세로가 아니라 **가로**를 늘린다

   16383 은 진짜 벽이라 문턱을 또 올려서는 못 넘는다. 그때 할 일은 폭을 512 → 1024 로
   넓히는 것이다(높이가 절반이 된다). 넣는 도구 넷이 폭 512 를 전제하고 있으므로
   그 넷을 함께 고쳐야 한다 — `adv-sheets.py` · `hero-sheets.py` · `inject-frames.py` ·
   `repack-atlas.py`. 이 검사가 그때를 미리 알리는 자다. */
